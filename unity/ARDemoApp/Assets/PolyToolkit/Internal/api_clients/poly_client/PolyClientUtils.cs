...
namespace PolyToolkitInternal.api_clients.poly_client {
  public static class PolyClientUtils {
    ...
    public static void GetRawFileBytesFromLocal(string filepath,GetRawFileDataBytesCallback callback)
    {
      if (!File.Exists(filepath))
      {
        Debug.Log("FILE NOT EXIST!!"+filepath);
        callback(PolyStatus.Error("File do not exist!"), null);
        return;
      }
      ...
      try
      {
        ...
        callback(PolyStatus.Success(), bytes);
      }
      catch (Exception e)
      {
        Debug.Log("ERROR READING "+filepath);
        callback(PolyStatus.Error(e.Message), null);
      }
    }
